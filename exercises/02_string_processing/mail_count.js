...
// input: string
// output: print to console (int, date range)
...
function parseEmail(email) {
  const sections = ["sender", "subject", "date", "recipient", "body"];

  const entriesArr = email
    .split("#/#")
    .map((content, i) => [sections[i], content.split(":").at(-1).trim()]);

  const res = Object.fromEntries(entriesArr);
  res.date = new Date(res.date); // assumes date in email data is local time zone

  return res;
}

function processEmails(emailData) {
  const processed = emailData.split("##||##").map(parseEmail);

  return processed;
}

function mailCount(emailData) {
  const emails = processEmails(emailData);
  const dates = emails.map((email) => email.date);

  const minDate = new Date(Math.min(...dates)).toDateString();
  const maxDate = new Date(Math.max(...dates)).toDateString();
